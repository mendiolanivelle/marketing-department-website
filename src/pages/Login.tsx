...
const otpSchema = z.object({
  otp: z.string().min(6, 'Code must be at least 6 characters').max(6, 'Code is 6 characters'),
})
...
export default function Login() {
  const { sendOtp, verifyOtp, configError } = useAuth()
  ...
  const onVerifyOtp = async (data: OtpFormData) => {
    setError(null)
    const { error } = await verifyOtp(sentEmail, data.otp)
    if (error) {
      setError(error.message)
    } else {
      ...
      <div className="fixed inset-0 overflow-hidden" style={{ backgroundColor: '#1B1A1C' }}>
        ...
        <div className="fixed inset-0 z-20 flex items-center justify-center p-4 sm:p-8">
          ...
          >
            ...
            <h2 className="text-2xl mb-1" style={{ color: '#CACDD7', fontWeight: 700 }}>{step === 'email' ? 'Sign in' : 'Check your email'}</h2>
            <p className="text-sm mb-8" style={{ color: '#CACDD7', fontWeight: 300, opacity: 0.6 }}>
              {step === 'email' ? 'Enter your email to receive a one-time code' : `We sent a code to ${sentEmail}`}
            </p>
            ...
            <form onSubmit={step === 'email' ? emailForm.handleSubmit(onSendOtp) : otpForm.handleSubmit(onVerifyOtp)} className="space-y-5">
              {step === 'email' ? (
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      style={{
                        ...
                        borderColor: emailForm.formState.errors.email ? '#FF5900' : '#3E4048',
                        color: '#CACDD7',
                      }}
                ...
                <div>
                  <label htmlFor="otp" className="block text-sm mb-2" style={{ color: '#CACDD7', fontWeight: 500, opacity: 0.8 }}>One-Time Code</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                      <svg className="w-5 h-5" style={{ color: '#FF5900', opacity: 0.6 }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-4 py-3.5 rounded-xl outline-none transition border tracking-[0.3em] text-center text-lg font-mono"
                      style={{
                        backgroundColor: '#1B1A1C',
                        borderColor: otpForm.formState.errors.otp ? '#FF5900' : '#3E4048',
                        color: '#CACDD7',
                      }}
                      placeholder="000000"
                    />
                  ...
                  <button type="button" onClick={() => { setStep('email'); setError(null) }} className="mt-3 text-sm transition hover:underline" style={{ color: '#FF5900', fontWeight: 500 }}>
              ...
              >
                {(step === 'email' ? emailForm.formState.isSubmitting : otpForm.formState.isSubmitting) ? (
                  <>
