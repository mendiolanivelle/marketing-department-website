    ...
    <div>
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-6 text-center" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold mb-3" style={{ color: 'var(--text-primary)' }}>Marketing Requests</h1>
          <p className="text-sm sm:text-base" style={{ color: 'var(--text-secondary)', fontWeight: 300 }}>
            Submit requests, track submissions, and reach out to the team
          </p>
        </div>
      </section>

      {/* Submit a Request */}
      <section className="py-14 sm:py-20 px-4 sm:px-6 text-center" style={{ backgroundColor: 'var(--bg-secondary)' }}>
        <div className="max-w-lg mx-auto">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ backgroundColor: 'var(--bg-card)' }}>
            <svg className="w-7 h-7" style={{ color: 'var(--accent)' }} fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
          </div>
          <h2 className="text-xl sm:text-2xl font-bold mb-3" style={{ color: 'var(--text-primary)' }}>Submit a Marketing Request</h2>
          <p className="text-sm mb-7" style={{ color: 'var(--text-secondary)', fontWeight: 300 }}>
            Use our structured form to submit your request. Include all necessary details so the marketing team can act quickly.
          </p>
          <a
            href="/#/submit-request"
            className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl text-white text-sm font-medium transition-all hover:-translate-y-0.5"
            style={{ backgroundColor: 'var(--accent)', boxShadow: '0 4px 12px rgba(255,89,0,0.25)' }}
          >
      ...
      {/* Submitted Requests */}
      <section className="py-14 sm:py-20 px-4 sm:px-6" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div className="flex-1">
              <h2 className="text-lg sm:text-xl font-bold" style={{ color: 'var(--text-primary)' }}>Submitted Requests</h2>
              <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)', fontWeight: 300 }}>{submitted.length} request{submitted.length !== 1 ? 's' : ''}</p>
            </div>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-16"><div className="animate-spin rounded-full h-7 w-7 border-b-2" style={{ borderColor: 'var(--accent)' }}></div></div>
          ) : submitted.length === 0 ? (
            ...
            <div className="space-y-3">
              {submitted.map((req, index) => (
                <div key={index} className="rounded-xl border theme-transition" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-primary)' }}>
                  <div
                    className="p-4 flex items-center justify-between gap-3 cursor-pointer hover:opacity-80 transition"
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs mt-0.5" style={{ color: 'var(--text-muted)', fontWeight: 300 }}>{req.name} · {req.department} · {new Date(req.created_at).toLocaleDateString()}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: `${priorityColors[req.priority] || '#6B7280'}15`, color: priorityColors[req.priority] || '#6B7280' }}>{req.priority}</span>
                    ...
                    <div className="px-4 pb-4 border-t" style={{ borderColor: 'var(--border-primary)' }}>
                      <div className="pt-4 space-y-3">
                        ...
                        <div className="flex justify-end pt-1">
                          <button onClick={(e) => { e.stopPropagation(); deleteRequest(index, req) }} disabled={deleting === index} className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg transition hover:bg-red-50" style={{ color: 'var(--text-muted)' }}>
