      ...
      <section className="py-20 px-6">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, index) => (
            <div key={index} className="bg-white p-9 rounded-2xl border border-gray-200 hover:border-indigo-200 hover:shadow-xl hover:-translate-y-1 transition-all">
              <div className="text-4xl mb-4" dangerouslySetInnerHTML={{ __html: service.icon }}></div>
              <h3 className="text-xl font-bold text-gray-900 mb-3">{service.title}</h3>
              <p className="text-gray-500 text-sm leading-relaxed mb-5">{service.description}</p>
              <ul className="flex flex-wrap gap-2">
                {service.features.map((feature, i) => (
                  <li key={i} className="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-md text-xs font-semibold">
                    {feature}
                  </li>
                ))}
              </ul>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="text-center p-8">
                <div className="text-4xl font-extrabold text-indigo-600 mb-4 opacity-30">{step.num}</div>
