...
export default function MeetingPlaybook() {
  ...
  const [links, setLinks] = useState<MeetingLink[]>(defaultLinks)
  ...
  const saveEdit = () => {
    ...
    if (target === 'link-label') {
      setLinks(prev => prev.map(l => l.id === id ? { ...l, label: editValue } : l))
    }
    ...
    if (target === 'checklist-text') {
      setChecklist(prev => prev.map(c => c.id === id ? { ...c, text: editValue } : c))
    }
  ...
  const addLink = () => {
    ...
    setLinks(prev => [...prev, { id, label: '🔗 New Link', url: '' }])
  }
  ...
  const addChecklistItem = () => {
    ...
    setChecklist(prev => [...prev, { id, text: 'New checklist item', checked: false }])
  }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8" style={{ backgroundColor: 'var(--bg-primary)', minHeight: '100vh' }}>
      ...
      {activeTab === 'active' && (
        <div>
          <h2 className="text-sm font-bold mb-4" style={{ color: 'var(--text-primary)' }}>Active Meeting</h2>

          {/* Section A: Important Links & Assets */}
          <div className="rounded-xl border p-5 mb-6" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-primary)' }}>
            <h3 className="text-xs font-bold mb-3" style={{ color: 'var(--text-primary)' }}>Section A: Important Links & Assets</h3>
            <div className="space-y-3">
              {links.map(link => (
                <div key={link.id} className="flex items-center gap-2 p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-secondary)' }}>
            ...
            <button className="text-xs mt-3 font-medium transition" style={{ color: 'var(--accent)' }} onClick={addLink}>➕ Add New Link/Asset</button>
          </div>

          {/* Section B: Pre-Meeting Checklist */}
          <div className="rounded-xl border p-5" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-primary)' }}>
            <h3 className="text-xs font-bold mb-3" style={{ color: 'var(--text-primary)' }}>Section B: Pre-Meeting Checklist</h3>
            <div className="space-y-2">
              {checklist.map(item => (
            ...
            <button className="text-xs mt-3 font-medium transition" style={{ color: 'var(--accent)' }} onClick={addChecklistItem}>➕ Add Custom Checklist Item</button>
          </div>
        </div>
      )}
