...
export default function PublicShowcase() {
  ...
  // Start sequence ╬ô├ç├╢ wait for images to actually load before slideshow
  useEffect(() => {
    preloadAll()
  ...
  // Auto-advance during slideshow ╬ô├ç├╢ stays on last slide, does not auto-close
  useEffect(() => {
  ...
  // Flash animation ╬ô├Ñ├å closing folder
  useEffect(() => {
  ...
  // Closing folder ╬ô├Ñ├å ended button
  useEffect(() => {
  ...
  return (
    <>
      ...
      >
        ...
        {/* ====== CLOSING ╬ô├ç├╢ zoom back to folder ====== */}
        {phase === 'closing' && (
          <div className="fixed inset-0 z-40 flex items-center justify-center" style={{ backgroundColor: '#1B1A1C' }}>
            ...
            >
              ...
              {/* Orange glow burst when closing ╬ô├ç├╢ fades as flap closes */}
              <div
                className="absolute"
                style={{
                  top: '50%',
        ...
        <div
          ...
          title=""
        >
          <span className="text-white/[0.06] text-[10px] hover:text-white/20 transition-colors duration-500 select-none">╬ô├╣├¬</span>
        </div>
      </div>
    </>
  )
