-- Reconcile only the atomic lead-ingestion RPCs without replaying migrations 017-051.
-- The three function bodies and their permissions intentionally match migration 044.

DO $$
BEGIN
  IF to_regclass('public.lead_files') IS NULL
    OR to_regclass('public.lead_rows') IS NULL
    OR (
      SELECT count(*)
      FROM information_schema.columns
      WHERE table_schema = 'public'
        AND table_name = 'lead_files'
        AND column_name = ANY (ARRAY['id', 'name', 'columns', 'source', 'created_at'])
    ) <> 5
    OR (
      SELECT count(*)
      FROM information_schema.columns
      WHERE table_schema = 'public'
        AND table_name = 'lead_rows'
        AND column_name = ANY (ARRAY['id', 'file_id', 'row_index', 'data'])
    ) <> 4
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'Atomic lead-ingestion reconciliation requires the canonical lead tables';
  END IF;

  IF to_regprocedure('public.import_lead_file(text,text[],text,jsonb)') IS NOT NULL
    OR to_regprocedure('public.append_calling_card_lead(jsonb)') IS NOT NULL
    OR to_regprocedure('public.route_lead_rows_to_duplicates(uuid[])') IS NOT NULL
  THEN
    RAISE EXCEPTION USING
      ERRCODE = '55000',
      MESSAGE = 'Atomic lead-ingestion reconciliation found unexpected existing RPCs';
  END IF;
END
$$;
